...
<div id="div-right">
    <article>
        ...
        ?>
        <h2 style="margin-top: 10px">Galeria-wybrane zdjęcia</h2>
        <ul id="galleryMenu">
            ...
            <li style="padding: 10px;border-bottom:  1px solid royalblue">Wybrane zdjęcia</li>
        </ul>
        <br>
        <br>
        <?php
            if(isset($selected)&&count($selected)>0)
                echo '<input type="submit" value="Usuń zaznaczone z zapamiętanych" form="deleteForm" class="blueButton">';
            else
                echo '<span style="color: red">Uwaga!</span> Nie wybrałeś żadnego zdjęcia';
        ?>
        <br>
        <form method="post" action="deleteSelected" id="deleteForm">
            <?php
                $i=1;
                $page=1;
                if(isset($gpage))
                ...
                foreach ($allEl as $image){
                    ...
                    if(!$checked||($image['status']==='private' && (!isset($logged) || $login!==$image['authorLogin'])))
                        continue;
                    else if($i>pageImgCount*($page-1)&&$i<=pageImgCount*$page)
                        echo showImage($image['_id'], $image['name'], $image['title'], $image['author'], $image['status']);
                    $i++;
                }
            ?>
        </form>
        ...
        <div id="pageEnd" style="text-align: center">
            <?php
                ...
                $href['0']="href='galeria-wybrane'";
                if($page==1)
                    $href['0']=$inactive;
                $href['1']="href='galeria-wybrane?strona=$prePage'";
                if($page==2)
                    $href['1']="href='galeria-wybrane'";
                if($page<2)
                    $href['1']=$inactive;
                $href['2']="href='galeria-wybrane?strona=$nexPage'";
                if($page==$maxPage)
                    $href['2']=$inactive;
                $href['3']="href='galeria-wybrane?strona=$maxPage'";
                if($page==$maxPage)
                    $href['3']=$inactive;
                echo
                "<a {$href['0']}>
